...
public:
    ...
    inline const double* getData_double() const { return data_double; }
    ...
    inline double getValue_double(
            unsigned int k, unsigned int j, unsigned int i) const
    { return data_double[INDEX3zyx_2(k, j, i, nlatsnlons, nlons)]; }

    inline double getValue_double(unsigned int n) const { return data[n]; }

    inline double getValue_double(MIndex3D idx) const
    { return data_double[INDEX3zyx_2(idx.k, idx.j, idx.i, nlatsnlons, nlons)]; }
    ...
    inline void setValue_double(
            unsigned int k, unsigned int j, unsigned int i, double v)
    { data_double[INDEX3zyx_2(k, j, i, nlatsnlons, nlons)] = v; }

    inline void setValue_double(unsigned int n, double v) { data_double[n] = v; }

    inline void setValue_double(MIndex3D idx, double v)
    { data_double[INDEX3zyx_2(idx.k, idx.j, idx.i, nlatsnlons, nlons)] = v; }
    ...
    inline double getLevel(int k) { return levels[k]; }
    inline double getLat(int j) { return lats[j]; }
    inline double getLon(int i) { return lons[i]; }
    ...
    virtual void setPressure(unsigned int k, unsigned int j,
                                    unsigned int i, float v)
    { Q_UNUSED(k); Q_UNUSED(j); Q_UNUSED(i); Q_UNUSED(v);}

    virtual float getSurfacePressure(unsigned int j,
                                    unsigned int i) const
    { Q_UNUSED(j); Q_UNUSED(i); return M_MISSING_VALUE;}

    virtual void setSurfacePressure(unsigned int j,
                                    unsigned int i, float v)
    { Q_UNUSED(j); Q_UNUSED(i); Q_UNUSED(v);}
    ...
    float getWestDataVolumeCorner_lon();

    float getEastDataVolumeCorner_lon();

    float getSouthDataVolumeCorner_lat();

    float getNorthDataVolumeCorner_lat();
    ...
    */
    ...
    /**
      ...
      latitude index.
     */
    double getDeltaLon_km(const int iLat) const;

    /**
      Computes the latitudinal grid point spacing in km.
     */
    double getDeltaLat_km() const;

    MVerticalLevelType getVerticalLevelType() { return leveltype; }

    MDataType getDataType() {return dataType; }

    void copyFloatDataToDouble();

    void copyDoubleDataToFloat();

    void initializeDoubleData();
...
public:
    ...
    float getPressure(unsigned int k, unsigned int j, unsigned int i) const override;
...
public:
    ...
    float getSurfacePressure(unsigned int j, unsigned int i) const override;

    void setSurfacePressure(unsigned int j, unsigned int i, float v) override;
    ...
    float getAkCoeff(unsigned int k);

    float getBkCoeff(unsigned int k);
...
public:
    ...
    void setPressure(unsigned int k, unsigned int j, unsigned int i, float v)
    override;
