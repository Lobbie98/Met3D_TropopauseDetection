...
NcVar NcCFVar::getVerticalCoordinateAuxiliaryPressure(
        ...
        int *indexOfVerticalLevel, bool disableGridConsistencyCheck)
{

//TODO (mr, 28Sept2021) -- the following identification of aux-p variables
// is a bit sketchy and needs to be revised. The main problem occurs when the
// pressure field is not stored in the same file as the current variable,
// in this case no checks are performed. A solution could be to revise the
// data loading procedure to first scan all variables, then let the user
// explicitly choose the pressure variable. This way the pressure variable
// would be known and corresponding checks could be performed.
    ...
    QStringList recognizedVerticalDimensionUnits;
    recognizedVerticalDimensionUnits
            << "Pa" << "hPa" << "bar" << "millibar" << "decibar"
            << "atmosphere" << "atm" << "m" << "metre" << "sigma";
    QStringList recognizedAxisDescriptors;
    recognizedAxisDescriptors << "Z" << "z";
    ...
    for (int i = 0; i < getDimCount(); i++)
    {
        // Get the variable that corresponds to the dimension name.
        NcVar coordinateVar = getParentGroup().getVar(getDim(i).getName());

        // Get "units" and "axis" attributes, if present.
        string units = "";
        try { coordinateVar.getAtt("units").getValues(units); }
        catch (NcException) {}
        fixZeroTermination(&units);
        string axis = "";
        try { coordinateVar.getAtt("axis").getValues(axis); }
        catch (NcException) {}
        fixZeroTermination(&axis);

        bool isVerticalCoordinate = false;
        ...
        // a vertical dimension unit.
        if (recognizedVerticalDimensionUnits.contains(QString::fromStdString(units))
                || recognizedAxisDescriptors.contains(QString::fromStdString(axis))
                || coordinateVar.isNull())
        {
            isVerticalCoordinate = true;
        }

        // The vertical coordinate might be not explicitly present for auxiliary
        // pressure fields, thus the coordinate variable can be Null.
        if (isVerticalCoordinate)
        {
            auxPressureVar = getParentGroup().getVar(
...
//TODO (mr, 28Sept2021) -- this needs to be changed to avoid errors due to
// wrongly identified aux-p variables.
            if (auxPressureVar.isNull())
            {
                // Check the size of the vertical dimension. If the size is
                // <3, do not accept as aux-p (needs to be 3D field with at
                // least 3 levels).
                int numLevels = getDim(i).getSize();
                if (numLevels < 3)
                {
                    throw MNcException(
                                "NcException",
                                "Number of vertical levels in aux-p variable "
                                "need to be at least 3.",
                                __FILE__, __LINE__);
                }

                LOG4CPLUS_WARN(mlog,
                               "\tWARNING: the auxiliary pressure variable <"
                               << auxiliary3DPressureField.toStdString()
                               << "> is not present in the file in which variable <"
                               << getName() << "> is stored and consistency cannot "
                               << "be checked -- assuming that <"
                               << getName() << "> is an aux-p "
                               << "variable to which the pressure variable <"
                               << auxiliary3DPressureField.toStdString()
                               << "> corresponds. The vertical dimension has "
                               << "been identified to be <" << getDim(i).getName()
                               << "> with <" << numLevels << "> levels. "
                               << "If this is wrong and errors occur, please check.");

                *pressureName = auxiliary3DPressureField;
                return auxPressureVar;
            }

            if (auxPressureVar.getDimCount() != getDimCount())
            {
                break;
            }
            ...
            {
                for (int j = 0; j < auxPressureVar.getDimCount(); j++)
                {
                    if (auxPressureVar.getDim(j).getName() != getDim(j).getName())
                    {
                        // Variable dimension doesn't match pressure field dimension.
                        isVerticalCoordinate = false;
                        break;
                    }
                }
            }
            ...
            {
                *pressureName = auxiliary3DPressureField;
                *indexOfVerticalLevel = i;
                break;
            }
            else
            {
                auxPressureVar = NcVar();
            }
        }
    }
