...
SpecificHumidityProcessor::SpecificHumidityProcessor()
    : MDerivedDataFieldProcessor(
          "specific_humidity",
...
void SpecificHumidityProcessor::compute(
        QList<MStructuredGrid *> &inputGrids, MStructuredGrid *derivedGrid)
{
    ...
    for (unsigned int n = 0; n < derivedGrid->getNumValues(); n++)
    {
        float specificHumidiy =inputGrids.at(0)->getValue(n);
            derivedGrid->setValue(n, specificHumidiy);
    }
}
...
MWetBulbPotentialTemperatureProcessor::MWetBulbPotentialTemperatureProcessor()
        : MDerivedDataFieldProcessor(
...
void MWetBulbPotentialTemperatureProcessor::compute(
        QList<MStructuredGrid *> &inputGrids, MStructuredGrid *derivedGrid)
{
    // input 0 = "air_temperature"
    // input 1 = "specific_humidity"
...
//! Potential Temperature".
                    // Pressure (hPa)
                    const double P_hPa = inputGrids[0]->getPressure(k, j, i);
                    // Hack! It can be that humidity values are negative! Computation will fail then!
                    // --> overcome this problem
                    if (q_kgkg < 0) { q_kgkg = 1E-10; }


                    float thetaW_K = MetRoutinesExperimental::computeWPT(T_K, P_hPa, q_kgkg);
                    derivedGrid->setValue(k, j, i, thetaW_K);
                }
            }
}
