...
unsigned int MStructuredGrid::getMemorySize_kb()
{
    ...
    return ( sizeof(MStructuredGrid)
             ...
             + ((data_double != nullptr) ? (nvalues * sizeof(double)) : 0)
...
QVector<QVector2D> MStructuredGrid::extractVerticalProfile(float lon, float lat)
{
    ...
    for (unsigned int k = 0; k < nlevs; k++)
    {
        ...
        //Julian M: Wenn es sich um Temperaturdaten(größer 200 in Kelvin) handelt, dann soillten die um Null grad herum verteilt werden.
        if(scalar > 200)
        {
            scalar = (scalar - 250.f) * 0.5f;
        }
...
float MStructuredGrid::getWestDataVolumeCorner_lon()
{
    return lons[0];
}

float MStructuredGrid::getEastDataVolumeCorner_lon()
{
    return lons[nlons-1];
}

float MStructuredGrid::getSouthDataVolumeCorner_lat()
{
    return lats[nlats-1];
}

float MStructuredGrid::getNorthDataVolumeCorner_lat()
{
    return lats[0];
}
...
double MStructuredGrid::getDeltaLon_km(const int iLat) const
{
    double phi = abs(getLats()[iLat]) * M_PI / 180.;
    double circumferenceLatitudeCircle_km = cos(phi) * 2. * M_PI
            * MetConstants::EARTH_RADIUS_km;
    double deltaGridpoints_km = circumferenceLatitudeCircle_km
            * (getDeltaLon() / 360.);
    return deltaGridpoints_km;
}


double MStructuredGrid::getDeltaLat_km() const
{
    double circumferenceLongitudeCircle_km = 2. * M_PI
            * MetConstants::EARTH_RADIUS_km;
    double deltaGridpoints_km = circumferenceLongitudeCircle_km
            * (getDeltaLat() / 360.);
    return deltaGridpoints_km;
}


void MStructuredGrid::copyFloatDataToDouble()
{
    initializeDoubleData();
    for (unsigned int i = 0; i < (nlevs * nlats * nlons); ++i)
    {
...
void MStructuredGrid::copyDoubleDataToFloat()
{
    if (dataType == DOUBLE)
    {
        for (unsigned int i = 0; i < (nlevs * nlats * nlons); ++i)
        {
            data[i] = (float) data_double[i];
        }
    }
}


void MStructuredGrid::initializeDoubleData()
{
    if (data_double == nullptr)
    {
        data_double = new double[nlevs * nlats * nlons];
    }
    dataType = DOUBLE;
}


void MStructuredGrid::deleteDoubleData()
{
    if (dataType == DOUBLE)
    {
        delete[] data_double;
...
float MLonLatHybridSigmaPressureGrid::getSurfacePressure(
        unsigned int j, unsigned int i) const
{
    return surfacePressure->getValue(j, i);
}


void MLonLatHybridSigmaPressureGrid::setSurfacePressure(
        unsigned int j, unsigned int i, float v)
{
    surfacePressure->setValue(j, i, v);
}
...
float MLonLatHybridSigmaPressureGrid::getAkCoeff(unsigned int k)
{
    return ak_hPa[k];
}

float MLonLatHybridSigmaPressureGrid::getBkCoeff(unsigned int k)
{
    return bk[k];
}
...
void MLonLatAuxiliaryPressureGrid::setPressure(
         unsigned int k, unsigned int j, unsigned int i, float v)
{
        auxPressureField_hPa->setValue(k, j, i, v);
}
