...
void MTask::addParent(MTask *task)
{
    ...
    if (task->isValid())
    {
        parents.append(task);
        task->addChild(this);
        involvesGPUTasks |= task->dependsOnGPUTasks();
...
void MTask::runGPUTask()
{
    emit gpuTaskProcessable(this);
}
...
bool MTask::hasGPUChild()

{
    QMutexLocker locker(&childrenMutex);
    for (int i = 0; i < children.size(); i++)
    {
        if (children[i]->isGPUTask()) { return true; }
    }
    return false;
}
