...
MSyncControl::MSyncControl(QString id, QWidget *parent) :
    ...
    jointlySettingInitValidDateTime(false)
{
...
{
    // Status variable "jointlySettingInitValidDateTime" will be unset in
    // onInitDateTimeChange().
    jointlySettingInitValidDateTime = true;

    // Valid time needs to be set first, see onValidDateTimeChange().
...
void MSyncControl::selectInitTimeAsListPosition(int posInAvailableInitTimes)
{
    ...
    {
        LOG4CPLUS_ERROR(mlog, "ERROR: Currently there are no init time available "
...
//      to BLOCK the GUI elements?
    ...
    // event of init time.
    if ( jointlySettingInitValidDateTime
         || ((ui->stepChooseVTITComboBox->currentIndex() == 2)
