...
#include <QLabel>
#include <QMouseEvent>
#include <src/actors/transferfunction1d.h>
...
//    camera.setOrigin(QVector3D(180., 0., 0.));
    ...
    displayShadowMapProperty = systemControl->getBoolPropertyManager()
            ->addProperty("display shadow map");
    systemControl->getBoolPropertyManager()
            ->setValue(displayShadowMapProperty, displayShadowMap);
    renderingGroupProperty->addSubProperty(displayShadowMapProperty);

    shadowMapLLCrnrProperty = systemControl->getPointFPropertyManager()->addProperty("shadow map SW crnr");
    renderingGroupProperty->addSubProperty(shadowMapLLCrnrProperty);

    shadowMapLLcrnr = QPointF(-90,20);
    systemControl->getPointFPropertyManager()->setValue(shadowMapLLCrnrProperty, shadowMapLLcrnr);

    shadowMapURCrnrProperty = systemControl->getPointFPropertyManager()->addProperty("shadow map NE crnr");
    renderingGroupProperty->addSubProperty(shadowMapURCrnrProperty);

    shadowMapURcrnr = QPointF(20, 80);
    systemControl->getPointFPropertyManager()->setValue(shadowMapURCrnrProperty, shadowMapURcrnr);
...
void MSceneViewGLWidget::onPropertyChanged(QtProperty *property)
{
    ...
    else if (property == displayShadowMapProperty)
    {
        MSystemManagerAndControl *sysMC =
                MSystemManagerAndControl::getInstance();
        auto* boolManager = sysMC->getBoolPropertyManager();
        displayShadowMap = boolManager->value(displayShadowMapProperty);
        updateGL();
    }

    else if (property == shadowMapLLCrnrProperty
            || property == shadowMapURCrnrProperty)
    {
        MSystemManagerAndControl *sysMC =
                MSystemManagerAndControl::getInstance();
        auto* pointFManager = sysMC->getPointFPropertyManager();

        shadowMapLLcrnr = pointFManager->value(shadowMapLLCrnrProperty);
        shadowMapURcrnr = pointFManager->value(shadowMapURCrnrProperty);
...
#define FOCUSSHADER_VERTEX_ATTRIBUTE 0
    ...
    loadShaders |= glRM->generateEffectProgram("oit_shader", oitShader);
    if (loadShaders)
    {
        oitShader->compileFromFile_Met3DHome("src/glsl/oit.fx.glsl");
    }

    loadShaders |= glRM->generateEffectProgram("shadow_map_debug", shadowMapShader);
    if (loadShaders)
    {
        shadowMapShader->compileFromFile_Met3DHome("src/glsl/shadow_map.fx.glsl");
    }
    ...
    // OIT: Initialize resources
    glGenBuffers(1, &atomicFragCount);
    glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, atomicFragCount); CHECK_GL_ERROR;
    glBufferData(GL_ATOMIC_COUNTER_BUFFER, sizeof(GLuint), nullptr, GL_DYNAMIC_COPY); CHECK_GL_ERROR;
    glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, 0);

    headBufferTexUnit = 2;
    const QString headBufferID = QString("head_buffer_#%1").arg(myID);

    // Support 4K monitors (in each dimension) by default (4K UHD)
    const uint maxScreenWidth = 3840;
    const uint maxScreenHeight = 2160;
    // Assume that on a 4K monitor, we only have 12 fragments per pixel
    const uint assumedFragmentsPerPixel = 12;
    // Compute number of potential fragments --> size of linked list buffer
    maxNumFragments = maxScreenWidth * maxScreenHeight * assumedFragmentsPerPixel;
    // Compute byte size of head buffer in GiB
    const float minimumSizeRequiredGB = maxNumFragments * sizeof(LinkedListElement) / 1024.f / 1024.f / 1024.f;

    LOG4CPLUS_DEBUG(mlog, QString("OIT: minimum size required for linked list buffer: %1 GiB")
            .arg(minimumSizeRequiredGB).toStdString());

    // Create head buffer texture (used to build linked list per pixel)
    headBufferTex = new GL::MTexture(headBufferID, GL_TEXTURE_2D, GL_R32UI,
                                         maxScreenWidth, maxScreenHeight);

    if (glRM->tryStoreGPUItem(headBufferTex)) {
        headBufferTex = dynamic_cast<GL::MTexture*>(glRM->getGPUItem(headBufferID));
    } else {
        delete headBufferTex;
        headBufferTex = nullptr;
    }

    if(headBufferTex)
    {
        //glRM->makeCurrent();
        headBufferTex->bindToLastTextureUnit();
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT); CHECK_GL_ERROR;
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_MIRRORED_REPEAT); CHECK_GL_ERROR;
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, maxScreenWidth, maxScreenHeight, 0,
                     GL_RED_INTEGER, GL_UNSIGNED_INT, nullptr); CHECK_GL_ERROR;
        glBindTexture(GL_TEXTURE_2D, 0);
    }

    // Create clear buffer as pixel buffer object
    // https://www.khronos.org/opengl/wiki/Pixel_Buffer_Object
    QVector<GLuint> nullDataPBO(maxScreenWidth * maxScreenHeight, { 0 });

    glGenBuffers(1, &headBufferClearPBO); CHECK_GL_ERROR;
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, headBufferClearPBO); CHECK_GL_ERROR;
    glBufferData(GL_PIXEL_UNPACK_BUFFER, maxScreenWidth * maxScreenHeight * sizeof(GLuint),
                 nullDataPBO.data(), GL_STATIC_DRAW); CHECK_GL_ERROR;
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0); CHECK_GL_ERROR;

    // Create buffer to store fragment linked list as SSBO
    const QString ssboLinkedListID =
            QString("linked_list_ssbo_#%1").arg(myID);

    linkedListSSBO = new GL::MShaderStorageBufferObject(
            ssboLinkedListID, sizeof(LinkedListElement),
            maxNumFragments);

    QVector<LinkedListElement> nullData(maxNumFragments, { 0 });
    linkedListSSBO->upload(nullData.data(), GL_DYNAMIC_DRAW);


    // Create head buffer texture (used to build linked list per pixel)
    headBufferShadowTexUnit = 6;
    const QString headBufferShadowID = QString("head_buffer_shadow_#%1").arg(myID);

    headBufferShadowTex = new GL::MTexture(headBufferShadowID, GL_TEXTURE_2D, GL_R32UI,
                                     maxScreenWidth, maxScreenHeight);

    if (glRM->tryStoreGPUItem(headBufferShadowTex)) {
        headBufferShadowTex = dynamic_cast<GL::MTexture*>(glRM->getGPUItem(headBufferShadowID));
    } else {
        delete headBufferShadowTex;
        headBufferShadowTex = nullptr;
    }

    if(headBufferShadowTex)
    {
        //glRM->makeCurrent();
        headBufferShadowTex->bindToLastTextureUnit();
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); CHECK_GL_ERROR;
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); CHECK_GL_ERROR;
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT); CHECK_GL_ERROR;
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_MIRRORED_REPEAT); CHECK_GL_ERROR;
    ...
    // Create clear buffer as pixel buffer object
    // https://www.khronos.org/opengl/wiki/Pixel_Buffer_Object
    QVector<GLuint> nullDataShadowPBO(maxScreenWidth * maxScreenHeight, { 0 });

    glGenBuffers(1, &headBufferShadowClearPBO); CHECK_GL_ERROR;
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, headBufferShadowClearPBO); CHECK_GL_ERROR;
    glBufferData(GL_PIXEL_UNPACK_BUFFER, maxScreenWidth * maxScreenHeight * sizeof(GLuint),
                 nullDataShadowPBO.data(), GL_STATIC_DRAW); CHECK_GL_ERROR;
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0); CHECK_GL_ERROR;

    // ================================

    // Shadow map: Initialize resources
    glGenFramebuffers(1, &shadowMapFBO);

    shadowMapTexUnit = 9; // assignTextureUnit();
    const QString shadowMapID = QString("shadow_map_#%1").arg(myID);

    shadowMap = new GL::MTexture(shadowMapID, GL_TEXTURE_2D,
                            GL_DEPTH_COMPONENT32, shadowMapRes, shadowMapRes);

    if (glRM->tryStoreGPUItem(shadowMap))
    {
        shadowMap = dynamic_cast<GL::MTexture*>(glRM->getGPUItem(shadowMapID));
    }
    else
    {
        delete shadowMap;
    ...
    if (shadowMap)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32,
                     ...
                     GL_DEPTH_COMPONENT, GL_FLOAT, 0); CHECK_GL_ERROR;
...
void MSceneViewGLWidget::paintGL()
{
    ...
    if (scene)
    {
        ...
        // OIT rendering:
        // Reset all resources used to gather and blend fragments

        // 1) Reset atomic fragment counter to 1 (= default value, 0 = no element in list)
        GLuint resetFragCount = 1;
        glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, atomicFragCount);
        glBufferSubData(GL_ATOMIC_COUNTER_BUFFER, 0, sizeof(GLuint), &resetFragCount); CHECK_GL_ERROR;
        glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, 0);

        // 2) Reset head buffer with pixel buffer object
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, headBufferClearPBO);
        headBufferTex->bindToTextureUnit(headBufferTexUnit); CHECK_GL_ERROR;
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, viewPortWidth, viewPortHeight, GL_RED_INTEGER, GL_UNSIGNED_INT, NULL); CHECK_GL_ERROR;
        glBindTexture(GL_TEXTURE_2D, 0); CHECK_GL_ERROR;
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0); CHECK_GL_ERROR;

        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, headBufferShadowClearPBO);
        headBufferShadowTex->bindToTextureUnit(headBufferShadowTexUnit); CHECK_GL_ERROR;
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, viewPortWidth, viewPortHeight, GL_RED_INTEGER, GL_UNSIGNED_INT, NULL); CHECK_GL_ERROR;
        glBindTexture(GL_TEXTURE_2D, 0); CHECK_GL_ERROR;
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0); CHECK_GL_ERROR;
        ...
        else
        {
            ...
            // Render each actor twice.
            // 1) Render the scene from the view of the light source

            QPointF center = (shadowMapLLcrnr + shadowMapURcrnr) / 2;

            QMatrix4x4 lightView;
            QMatrix4x4 lightProj;

            QVector3D centerShadowMap(center.x(), center.y(), ztop * 2);

            lightView.lookAt(centerShadowMap, QVector3D(center.x(), center.y(), zbot),
                             QVector3D(0, 1, 0));
            lightProj.ortho(shadowMapLLcrnr.x() - center.x(),
                            shadowMapURcrnr.x() - center.x(),
                            shadowMapLLcrnr.y() - center.y(),
                            shadowMapURcrnr.y() - center.y(),
                            0, ztop * 2);

            lightOrthoProjectionMatrix = lightProj * lightView;

            glBindFramebuffer(GL_FRAMEBUFFER, shadowMapFBO); CHECK_GL_ERROR;
            ...
            // --> disable draw buffer and read buffer.
            ...
            glViewport(0, 0, shadowMapRes, shadowMapRes); CHECK_GL_ERROR;
            glBindFramebuffer(GL_FRAMEBUFFER, shadowMapFBO); CHECK_GL_ERROR;
            // Clear all set depth values in the framebuffer.
            glClear(GL_DEPTH_BUFFER_BIT); CHECK_GL_ERROR;

            inShadowMappingMode = true;

            //QVector3D oldOrigin = camera.getOrigin();
            ...
            {
                if (dynamic_cast<MTransferFunction1D*>(actor)) { continue; }
            ...
            foreach (MActor* actor, scene->getRenderQueue())
            {
                ...
                actor->renderTransparency(this);
            }

            if (multisamplingEnabled) { glEnable(GL_MULTISAMPLE); }

            inShadowMappingMode = false;

            // Restore the old viewport and use the default frame buffer.
            glViewport(0, 0, viewPortWidth, viewPortHeight); CHECK_GL_ERROR;
            glBindFramebuffer(GL_FRAMEBUFFER, 0); CHECK_GL_ERROR;

            // 2) Render scene and shadow objects using the shadow map
            foreach (MActor* actor, scene->getRenderQueue())
            {
                if (singleInteractionActor != nullptr &&
                        singleInteractionActor->getName() == actor->getName())
...
//        if (!renderLabelsWithDepthTest) glEnable(GL_DEPTH_TEST);
        ...
        // Render all translucent actors after all opaque geometry has been rendered
        // Disable depth tests and disable multisampling, as this does not
        // work with OIT.
        glDisable(GL_MULTISAMPLE);
    ...
    // Sort linked list buffer and blend colours over framebuffer
    oitShader->bindProgram("OIT");

    oitShader->setUniformValue("headBuffer", 0);
    glBindImageTexture(0, headBufferTex->getTextureObject(), 0, GL_FALSE, 0, GL_READ_ONLY, GL_R32UI); CHECK_GL_ERROR;

    oitShader->setUniformValue("headBufferShadow", 1);
    glBindImageTexture(1, headBufferShadowTex->getTextureObject(), 0, GL_FALSE, 0, GL_READ_ONLY, GL_R32UI); CHECK_GL_ERROR;

    linkedListSSBO->bindToIndex(0); CHECK_GL_ERROR;
    ...
    if (displayShadowMap)
    {
        ...
        shadowMapShader->setUniformValue("shadowMap", shadowMapTexUnit); CHECK_GL_ERROR;

        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); CHECK_GL_ERROR;
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4); CHECK_GL_ERROR;
    }

    // Restore Settings like before
    glEnable(GL_DEPTH_TEST);
...
//    settings->setValue("verticalScale", northArrow.verticalScale);
    settings->setValue("horizontalScale",  sysMC->getDecoratedDoublePropertyManager()
                       ->value(northArrow.horizontalScaleProperty));
    settings->setValue("verticalScale",  sysMC->getDecoratedDoublePropertyManager()
                       ->value(northArrow.verticalScaleProperty));
...
void MSceneViewGLWidget::setOITUniforms(std::shared_ptr<GL::MShaderEffect> effect) const
{
    // OIT: Bind oit specific variables
    // 1) Bind atomic counter
    glBindBufferBase(GL_ATOMIC_COUNTER_BUFFER, 0, atomicFragCount);
    CHECK_GL_ERROR;
    // 2) Bind head buffer
    effect->setUniformValue(
            "headBuffer", 0);
    glBindImageTexture(0, headBufferTex->getTextureObject(), 0,
                       GL_FALSE, 0, GL_READ_WRITE, GL_R32UI); CHECK_GL_ERROR;
    // 3) Bind head buffer for shadows
    effect->setUniformValue(
            "headBufferShadow", 1);
    glBindImageTexture(1, headBufferShadowTex->getTextureObject(), 0,
                       GL_FALSE, 0, GL_READ_WRITE, GL_R32UI); CHECK_GL_ERROR;
    // 3) Bind linked list
    linkedListSSBO->bindToIndex(0); CHECK_GL_ERROR;
    // 4) Bind max. number of fragments
    effect->setUniformValue("maxNumFragments", maxNumFragments);
}
