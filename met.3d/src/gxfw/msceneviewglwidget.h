...
public:
    ...
    { return (inShadowMappingMode) ? &lightOrthoProjectionMatrix :
    ...
    QMatrix4x4* getLightOrthoProjectionMatrix()
    {
        return &lightOrthoProjectionMatrix;
    }
    ...
    bool isMultisamplingEnabled() const { return multisamplingEnabled; }
    ...
    void          setOITUniforms(std::shared_ptr<GL::MShaderEffect> effect) const;

    GLuint        getOITFragCounter() const { return atomicFragCount; }
    GL::MTexture* getOITHeadBuffer() const { return headBufferTex; }
    GL::MShaderStorageBufferObject* getOITLinkedList() const { return linkedListSSBO; }
    GLuint        getOITMaxNumFragments() const { return maxNumFragments; }

    GL::MTexture* getShadowMap() const { return shadowMap; }
    int           getShadowMapTexUnit() const { return shadowMapTexUnit; }
    bool shadowMappingInProgress() const { return inShadowMappingMode; }
...
private:
    ...
    std::shared_ptr<GL::MShaderEffect> oitShader;
    std::shared_ptr<GL::MShaderEffect> shadowMapShader;

    // OIT related buffers:
    // Atomic counter to count the total number of fragments
    GLuint                              atomicFragCount;

    // Head buffer texture
    GL::MTexture*                       headBufferTex;
    int                                 headBufferTexUnit;
    // Pixel buffer object to clear head buffer
    GLuint                              headBufferClearPBO;

    GL::MTexture*                       headBufferShadowTex;
    int                                 headBufferShadowTexUnit;
    // Pixel buffer object to clear head buffer
    GLuint                              headBufferShadowClearPBO;
    // Linked list
    struct LinkedListElement
    {
        uint32_t color, depth, prev;
    };

    GL::MShaderStorageBufferObject*     linkedListSSBO;
    GLuint                              maxNumFragments;

    // Shadow mapping for fronts and jet cores
    GL::MTexture*       shadowMap;
    int                 shadowMapTexUnit;
    GLuint              shadowMapFBO;
    GLuint              shadowMapRes;
    QMatrix4x4          lightOrthoProjectionMatrix;
    bool                inShadowMappingMode;
    ...
    QtProperty *displayShadowMapProperty;
    bool        displayShadowMap;
    QtProperty *shadowMapLLCrnrProperty;
    QtProperty *shadowMapURCrnrProperty;
    QPointF    shadowMapLLcrnr;
    QPointF    shadowMapURcrnr;
