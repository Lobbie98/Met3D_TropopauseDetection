...
public:
    ...
    /**
      If the actor is enabled (property @ref enabled is set to true) and translucent,
      render the actor in the current OpenGL context using order-independent transparency.
      Calls the virtual method @ref
      renderTransparencyToCurrentContext() to perform the actual rendering
      in the derived classes. Do not overwrite this method, but re-implement
      @ref renderToCurrentContext() instead.
      */
    void renderTransparency(MSceneViewGLWidget *sceneView);
...
protected:
    ...
    virtual void renderTransparencyToCurrentContext(MSceneViewGLWidget *sceneView) = 0;
