        ...
        const QString &filename)
{
    ...
    if (met3DWorkingDirectory != ""
            && !(QDir().mkpath(met3DWorkingDirectory)
                 ...
                 && QFileInfo::exists(met3DWorkingDirectory)))
    {
        //QMessageBox msgBox;
...
//                          " configuration file.");
        LOG4CPLUS_WARN(mlog, "No write access to "
                       << met3DWorkingDirectory.toStdString()
                       << "'.\nUsing default path '"
                       << QDir::home().absoluteFilePath("met3d").toStdString()
                       <<  "'.\nPlease check setting in frontend"
                          " configuration file.");
        //msgBox.exec();
...
//                          " configuration file.");
        LOG4CPLUS_WARN(mlog, "No write access to "
                       << sessionDirectory.toStdString()
                       << "'.\nUsing default path '"
                       << QDir::home().absoluteFilePath("met3d/sessions").toStdString()
                       <<  "'.\nPlease check setting in frontend"
                          " configuration file.");
//        msgBox.exec();
    ...
    QString batchModeAnimationStartTimeDataSource =
            config.value("startAnimationAtMostRecentInitTimeOfDataSource", "").toString();
    ...
    LOG4CPLUS_DEBUG(mlog, "  batch mode animation starts at most recent "
                          "available init time of data source "
