...
void MTransferFunction1D::renderTransparencyToCurrentContext(MSceneViewGLWidget *sceneView)
{
    // First draw the colourbar itself. glPolygonOffset is used to displace
    // the colourbar's z-value slightly to the back, to that the frame drawn
    // afterwards is rendered correctly.
    colourbarShader->bindProgram("colourbarTFOIT");
    ...
    // OIT specific --> set required OIT variables
    // (head buffer, linked list, frag count)
    sceneView->setOITUniforms(colourbarShader);
