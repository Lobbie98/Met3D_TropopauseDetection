      ...
      shadowElevation_hPa(1049.7)
{
    ...
    renderModeNames << "entire trajectories as tubes"
                    ...
                    << "entire trajectory, decreasing radius from valid time";
    ...
    tubeMinRadiusProperty = addProperty(DECORATEDDOUBLE_PROPERTY, "tube min radius",
                                     renderingGroupProperty);
    properties->setDDouble(tubeMinRadiusProperty, tubeMinRadius,
                           0.01, 1., 2, 0.1, " (world space)");
    ...
    shadowElevationProperty = addProperty(DECORATEDDOUBLE_PROPERTY, "shadow elevation",
                                       renderingGroupProperty);
...
void MTrajectoryActor::reloadShaderEffects()
{
    ...
    compileShadersFromFileWithProgressDialog(
                tubeTimeShader,
                "src/glsl/trajectory_tubes_time.fx.glsl");
    compileShadersFromFileWithProgressDialog(
                tubeTimeShadowShader,
                "src/glsl/trajectory_tubes_time_shadow.fx.glsl");
...
void MTrajectoryActor::saveConfiguration(QSettings *settings)
{
    ...
    settings->setValue("tubeMinRadius", tubeMinRadius);
...
void MTrajectoryActor::loadConfiguration(QSettings *settings)
{
    ...
    properties->mDDouble()->setValue(
                tubeMinRadiusProperty,
                settings->value("tubeMinRadius", 0.1).toDouble());
...
void MTrajectoryActor::initializeActorResources()
{
    ...
    loadShaders |= glRM->generateEffectProgram("trajectory_spheresshadow",
                                               positionSphereShadowShader);
    loadShaders |= glRM->generateEffectProgram("trajectory_tube_time",
                                               tubeTimeShader);
    loadShaders |= glRM->generateEffectProgram("trajectory_tube_time_shadow",
                                               tubeTimeShadowShader);
...
//        updateSyncPropertyColourHints();
    ...
    else if (property == tubeMinRadiusProperty)
    {
        tubeMinRadius = properties->mDDouble()->value(tubeMinRadiusProperty);
        if (suppressActorUpdates()) return;
        emitActorChangedSignal();
    }
    ...
    else if (property == shadowElevationProperty)
    {
...
void MTrajectoryActor::renderToCurrentContext(MSceneViewGLWidget *sceneView)
{
         ...
         || (renderMode == BACKWARDTUBES_AND_SINGLETIME))
    {
             ...
             t++)
        {
            ...
            if (shadowEnabled)
            {
                ...
                tubeShadowShader->setUniformValue("shadowWorldZHeight", shadowWorldZ);
    ...
    if (renderMode == TIME_DEPENDEND_RADIUS)
    {
        for (int t = 0; t < (precomputedDataSource ? 1 : seedActorData.size());
             t++)
        {
            ...
            tubeTimeShader->setUniformValue(
                    "mvpMatrix",
                    *(sceneView->getModelViewProjectionMatrix()));
            tubeTimeShader->setUniformValue(
                    "pToWorldZParams",
            ...
            tubeTimeShader->setUniformValue(
                    "minRadius",
                    GLfloat(tubeMinRadius));

            tubeTimeShader->setUniformValue(
                    "timeStepsToReduceRadius",
                    GLint(100));
            ...
            if (shadowEnabled)
            {
                ...
                tubeTimeShadowShader->bind();

                tubeShadowShader->setUniformValue(
                        "mvpMatrix",
                        *(sceneView->getModelViewProjectionMatrix()));
                tubeTimeShadowShader->setUniformValue(
                        "pToWorldZParams",
                        sceneView->pressureToWorldZParameters());
                tubeTimeShadowShader->setUniformValue(
                        "lightDirection",
                ...
                GLfloat shadowWorldZ = sceneView->worldZfromPressure(shadowElevation_hPa);
                tubeTimeShadowShader->setUniformValue("shadowWorldZHeight", shadowWorldZ);
...
void MTrajectoryActor::renderTransparencyToCurrentContext(MSceneViewGLWidget *sceneView)
{
    Q_UNUSED(sceneView);
}
...
//    ensembleModeProperty->setEnabled(enable);
    ...
    tubeMinRadiusProperty->setEnabled(enable);
