...
shader FSmainOIT(in VStoFS Input)//, out vec4 fragColour)
{
    vec4 color = texture(transferTexture, Input.texCoord);
    if (!enableAlpha) color.a = 1.0;

    insertNewFragmentOIT(color, gl_FragCoord.xyz);
    // discard fragment after fragment stage
    discard;
}
...
program colourbarTFOIT
{
    vs(430)=VSmain();
    fs(430)=FSmainOIT() : in(early_fragment_tests);
};
