...
uniform float minRadius;                 // min radius of the tube in world space
uniform int   timeStepsToReduceRadius;   // time steps for reduction of tube radius around valid time
uniform int   validTimePosIndex;         // valid time step index around which tube radius will be reduces
              ...
              out float gs_scalar)
{
    ...
    float radius2 = radius0;
    float radius3 = radius0;
...
//    int reduceRadiusFirstIdx = validTimePosIndex - timeStepsToReduceRadius;
//    int reduceRadiusLastIdx = validTimePosIndex + timeStepsToReduceRadius;

//    if (mod(vs_vertexID[0], reduceRadiusFirstIdx) > 500)
//    {
//        radius0 = radius;
//    }
//    if (mod(vs_vertexID[1], reduceRadiusFirstIdx) > 500)
//    {
//        radius1 = radius;
//    }
//    if (mod(vs_vertexID[2], reduceRadiusFirstIdx) > 500)
//    {
//        radius2 = radius;
//    }
//    if (mod(vs_vertexID[3], reduceRadiusFirstIdx) > 500)
//    {
//        radius3 = radius;
//    }
    ...
    if (mod(vs_vertexID[0], numObsPerTrajectory) == 0)
    {
       ...
       gs_worldSpacePosition = position1 + radius1 * gs_worldSpaceNormal;
       gl_Position           = mvpMatrix * vec4(gs_worldSpacePosition, 1.);
       gs_scalar             = scalar1;
       EmitVertex();

       gs_worldSpaceNormal   = normal0;
       gs_worldSpacePosition = position0 + radius0 * gs_worldSpaceNormal;
       gl_Position           = mvpMatrix * vec4(gs_worldSpacePosition, 1.);
       gs_scalar             = scalar0;
       EmitVertex();
   ...
   /* generateTube(position1, position2, normal1, normal2,
    ...
    gs_worldSpacePosition = position2 + radius2 * gs_worldSpaceNormal;
    gl_Position           = mvpMatrix * vec4(gs_worldSpacePosition, 1.);
    gs_scalar             = scalar2;
    EmitVertex();
    ...
    if (mod(vs_vertexID[3], numObsPerTrajectory) == (numObsPerTrajectory - 1))
    {
       ...
       gs_worldSpacePosition = position3 + radius3 * gs_worldSpaceNormal;
       gl_Position           = mvpMatrix * vec4(gs_worldSpacePosition, 1.);
       gs_scalar             = scalar3;
       EmitVertex();
