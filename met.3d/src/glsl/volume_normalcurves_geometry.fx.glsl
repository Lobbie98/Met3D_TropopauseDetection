...
shader FSShadowOIT(in GStoFS Input, out vec4 fragColor)
{
    if (Input.valuePS == -1) { discard; }

    fragColor = shadowColor;

    insertNewFragmentOIT(shadowColor, gl_FragCoord.xyz, true);
...
{
    vs(430)=VSmain();
    gs(430)=GSLine() : in(lines_adjacency), out(line_strip, max_vertices = 2);
    fs(430)=FSLine();
};
...
{
    vs(430)=VSmain();
    gs(430)=GSBox() : in(lines_adjacency), out(triangle_strip, max_vertices = 128);
    fs(430)=FSGeom();
};
...
{
    vs(430)=VSShadow();
    gs(430)=GSTubeShadow() : in(lines_adjacency), out(triangle_strip, max_vertices = 128);
    fs(430)=FSShadow();
};

program TubeShadowOIT
{
    vs(430)=VSShadow();
    gs(430)=GSTubeShadow() : in(lines_adjacency), out(triangle_strip, max_vertices = 128);
    fs(430)=FSShadowOIT()  : in(early_fragment_tests);
};
