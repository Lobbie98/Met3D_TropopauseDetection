...
shader GSmain(out GStoFS outStruct)
{
    ...
    if (ul_sectionGrid.r == MISSING_VALUE || ur_sectionGrid.r == MISSING_VALUE
            || ll_sectionGrid.r == MISSING_VALUE || lr_sectionGrid.r == MISSING_VALUE)
    {
        EndPrimitive();
        return;
    }
    ...
    float lon_l = texelFetch(path, 2*ul.x    , 0).a;
    float lat_l = texelFetch(path, 2*ul.x + 1, 0).a;
    float lon_r = texelFetch(path, 2*ur.x    , 0).a;
    float lat_r = texelFetch(path, 2*ur.x + 1, 0).a;
